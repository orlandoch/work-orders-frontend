<template>
  <div v-if="supplier">
    <div class="flex items-center gap-3 mb-4">
      <Button icon="pi pi-arrow-left" text rounded @click="$router.push('/suppliers')" />
      <h1 class="text-xl font-bold">{{ supplier.full_name }}</h1>
      <span class="ml-auto flex gap-2">
        <Button label="Editar" icon="pi pi-pencil" outlined @click="$router.push(`/suppliers/${supplier.id}/edit`)" />
      </span>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <Card>
        ...
        <template #content>
          <dl class="text-sm space-y-2">
            <div class="flex justify-between"><dt class="text-gray-500">Documento</dt><dd>{{ supplier.document_type }} {{ supplier.document_number }}</dd></div>
            <div class="flex justify-between"><dt class="text-gray-500">Activo</dt><dd><i :class="supplier.is_active ? 'pi pi-check text-green-500' : 'pi pi-times text-red-400'"></i></dd></div>
          </dl>
        </template>
      </Card>
      ...
      <Card>
        ...
        <template #content>
          <p class="text-sm text-gray-400">Próximamente: productos, compras, movimientos</p>
        </template>
      </Card>
    </div>
  </div>
...
const contactGroups = computed(() => {
  if (!supplier.value?.contact_details) return []
  ...
  for (const cd of supplier.value.contact_details) {
