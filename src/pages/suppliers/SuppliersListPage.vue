<template>
  <div>
    <div class="page-header"><Button label="Nuevo Proveedor" icon="pi pi-plus" @click="$router.push('/suppliers/new')" /></div>
    <DataTable scrollable scrollHeight="flex" :value="items" :loading="loading" paginator :rows="15" stripedRows
      ...
      <Column header="" body-style="text-align:right; min-width: 130px">
        <template #body="{data}">
          ...
          <Button icon="pi pi-pencil" text rounded @click.stop="$router.push(`/suppliers/${data.id}/edit`)" />
...
function del(c: any) {
  confirm.require({
    ...
    accept: async () => {
      try {
        await api.delete(`/suppliers/${c.id}`)
